...
import { dontKnow, skill } from "./src/data";
...
// const systemPrompt = `Answer questions only using the provided data: ${data}. If the requested information is not present in the data, respond with: 'I only have information about Terry...', DON'T ANSWER ANYTHING ELSE. If the question is English, MUST answer in English. If the question is Traditional Chinese, MUST answer in Traditional Chinese. ONLY append the mark ${mark.contact} to the response if the question has some wordings like 'contact' or 'reach out'; otherwise, omit it—especially especially if the user is inquiring about Terry's skills.`;
...
app.get("/ask", async (req, res) => {
  try {
    ...
    // console.log("Message content:", aiResponse.choices[0].message.content);
